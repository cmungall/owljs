/* Namespace: obol
  ...
  Each token in a token list is either a terminal (a string) or a non-terminal.
  Currently the only non-terminals allowed are variables, objects keyed by
  minimally 'name', and optionally 'type.
  ...
  This matches any class whose label, synonym, etc ends with "differentiation". The
  start of the string must correspond to the label of a cell type.
  Each match generates a class expression.
...
/* Function: add
 *
 * Class-level function to add a parse rule
 *
 * Arguments:
 * - r : a rule object
 *
...
Obol.prototype.isSubsumersOnly = function() {
    if (this.owl.config.isSubsumersOnly == null) {
        return false;
    }
    return this.owl.config.isSubsumersOnly;
}
Obol.prototype.isRespectCase = function() {
    if (this.owl.config.isRespectCase == null) {
        return false;
    }
    return this.owl.config.isRespectCase;
}
 ...
 *
 */
Obol.prototype.restrictProperties = function(ps) {
    var t = this;
    var owl = this.owl;
...
Obol.prototype.getLabelEntriesForObject = function(c) {
    var owl = this.owl;
    var elist = [];
...
*/
Obol.prototype.parse = function(qstr, head, baseScore, property) {
    var ml = [];
    ...
    if (typeof qstr != 'string') {
        ...
        for (var k in elist) {
            var e = elist[k];
            ml = ml.concat(this.parse(e.label, head, e.score, e.property));
        }
        return ml;
    }
    ...
    for (var k in Obol.rules) {
        ...
        if (head == null || r.head == head) {
            ...
            if (m != null) {
                ...
                if (x.getClassesInSignature().contains(qstr)) {
                    m = null;
                }
                else {
                    var score = m.score;
...
Obol.prototype.parseWith = function(qstr, rule) {
    ...
    this.log("Passed pre-test: "+qstr+" in "+head);
...
Obol.prototype.writeOboFile = function(fn, results) {
    var fs = require('fs');
...
Obol.prototype.writeObo = function(s, r) {
    var axiom = r.axiom;
    var obj = r.obj;
