...
Obol.prototype.parse = function(qstr, head) {
    ...
    for (var k in Obol.rules) {
        ...
        if (head == null || r.head == head) {
            ...
            if (m != null) {
                this.log("b="+m.bindings);
                this.log("labelObjectMap="+this.labelObjectMap+" "+this.labelObjectMap.part_of);
...
Obol.prototype.matchTokens = function(qstr, toksToMatch) {
    ...
    var m = this.matchToken(qstr, toksToMatch[0], remainingTokens);
    if (m == null) {
        return null;
    }
