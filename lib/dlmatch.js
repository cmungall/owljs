 ...
 *
 * In addition the "axiom" key is also filled in.
 *
 * Arguments:
 *  - q: a query template
 *
 ...
 *
 * Returns:
 *  { newAxioms: <owlapi.OWLAxiom> [], rmAxioms: <owlapi.OWLAxiom> []}
 */
...
DLMatch.prototype.findAndReplaceOrExtend = function(q, rfunc, isRemove) {
    var owl = this.owl;
    var axioms = owl.getAllAxioms();
    return this.findAndReplaceOrExtendInAxioms(q, rfunc, isRemove, axioms);
}

DLMatch.prototype.findAndReplaceOrExtendInAxioms = function(q, rfunc, isRemove, axioms) {
    var owl = this.owl;
    this.log("Testing: "+rfunc+" vs #axioms = "+axioms.length);
    var newAxioms = [];
    var rmAxioms = [];
    var replacements = [];
    for (var k in axioms) {
...
DLMatch.prototype.matchVariable = function(x, q) {
    var qv = this.getVariable(q);
    if (qv.subClassOf != null) {
        qv.constraint =
            function(w, owl) {
                return owl.isInferredSubClassOf(w, qv.subClassOf)
        };
...
// Args: targetElementArray, queryElementArray
...
DLMatch.prototype.matchSet = function(x, q) {
    ...
    var requiresCompleteMatch = true;

    // create match matrix
    for (var qi=0; qi<q.length; qi++) {
        var subq = q[qi];
        if (subq == '...') {
            requiresCompleteMatch = false;
            // TODO
            break;
        }
        //this.log(" QI: "+qi+" "+subq);
...
DLMatch.prototype.log = function(msg) {
    if (this.config != null && this.config.logLevel > 4) {
        console.log(msg);
    }
}
DLMatch.prototype.info = function(msg) {
...
DLMatch.prototype.fromTemplate = function(x, vmap, opts) {
    var obj = this.owl.toAxiomaticJSON(x);
    var m = this.replaceObjectsWithVariables(obj, vmap);
    return m;
}

DLMatch.prototype.mapToVar = function(vmap, obj, v) {
    vmap[obj.getIRI().toString() + ""] = v;
}

DLMatch.prototype.replaceObjectsWithVariables = function(obj, vmap) {
    if (vmap[obj] != null) {
        return vmap[obj];
    }
    var t = this;
    if (obj.map != null){ 
        return obj.map(function(x){return t.replaceObjectsWithVariables(x, vmap)});
    }
    ...
    if (typeof obj == 'object') {
        ...
        for (var k in obj) {
            if (k == 'type') {
                var cn = this.cmap()[obj[k]];
                if (cn != null) {
                    obj2.a = cn;
                    continue;
                }
                console.warn("Cannot map: "+obj[k]);
            }
            obj2[k] = this.replaceObjectsWithVariables(obj[k], vmap);
        }
        return obj2;
    }
    return obj;
...
DLMatch.prototype.cmap = function(obj, vmap) {
    return {
        "SubClassOf" : OWLSubClassOfAxiom,
        "ObjectSomeValuesFrom" : OWLObjectSomeValuesFrom,
    };
}
