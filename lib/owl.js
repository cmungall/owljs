 ...
 * Returns: <OWLAxiom> []
 */
OWL.prototype.grepAxioms = function(grepFunc) {
...
OWL.prototype.find = function(key) {
    var objs = this.getOntology().getClassesInSignature(true).toArray();
    for (var k in objs) {
        var obj = objs[k];
...
OWL.prototype.keyMatches = function(key, obj) {
    if (this.getLabel(obj).equals(key)) {
        return true;
    }
    return false;
}
 ...
 * Returns: OWLFrame
 */
OWL.prototype.generateXP = function(genus, relation, diff) {
    var iri = this.genIRI();
    var id = iri.toString();
...
OWLFrame.prototype.stamp = function() {
    ...
    if (this.generator.defaultSlotMap != null) {
        for (var k in this.generator.defaultSlotMap) {
